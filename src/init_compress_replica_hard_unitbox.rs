...
fn main() {
    ...
    let mut sim = Simulation::new_from_config_unitbox(config);
    ...
    for step in 0..config.step_max {
        ...
        let forces_a = sim.f_system_hertz_hard();
        let forces_b = sim.f_system_hertz_hard_sigma(&sigma_b);
        ...
        sim.langevin_step_with_forces_w_hard(&forces_a, &w);
...
fn get_final_sigma(sim: &Simulation, config: &InitCompressReplicaConfig) -> f64 {
    ...
    let final_sigma = initial_l/final_l*(&sim.sigma);
    final_sigma
}
